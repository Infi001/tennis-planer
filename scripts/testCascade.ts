...
console.log('\n--- TEST 4: 2. Springer declines offer (Cascade to Prio 3 -> Frei) ---');
const weekSp2Declined: TrainingWeek = JSON.parse(JSON.stringify(weekSp1Declined));
weekSp2Declined.springer2.status = 'declined';
res = calculateStandbyCascade(weekSp2Declined);
assert(res.totalOpenSpots === 1, 'Total open spots is 1');
assert(res.springer1.status === 'declined', 'Springer 1 is declined');
assert(res.springer2.status === 'declined', 'Springer 2 is declined');
assert(res.frei.status === 'offered', 'Offer automatically cascades to Frei (Prio 3)');
assert(res.openForAnyoneCount === 0, 'Open for anyone is 0');
assert(res.activeOfferedPrios.length === 1 && res.activeOfferedPrios[0] === 3, 'Active offered prio is [3]');

console.log('\n--- TEST 5: Frei declines offer (Cascade to Prio 4 -> Alle Spieler) ---');
const weekFreiDeclined: TrainingWeek = JSON.parse(JSON.stringify(weekSp2Declined));
weekFreiDeclined.frei.status = 'declined';
res = calculateStandbyCascade(weekFreiDeclined);
assert(res.totalOpenSpots === 1, 'Total open spots is 1');
assert(res.springer1.status === 'declined', 'Springer 1 is declined');
assert(res.springer2.status === 'declined', 'Springer 2 is declined');
assert(res.frei.status === 'declined', 'Frei is declined');
assert(res.openForAnyoneCount === 1, 'Spot is now 100% open for anyone in the club (Prio 4)');
...
res = calculateStandbyCascade(week2Declines);
...
console.log('\n--- TEST 7: 3 spots open -> Springer 1, Springer 2, AND Frei all offered ---');
const week3Declines: TrainingWeek = JSON.parse(JSON.stringify(baseWeek));
week3Declines.slots['18:00-19:00'][0].status = 'declined';
week3Declines.slots['19:00-20:00'][0].status = 'declined';
week3Declines.slots['20:00-21:00'][0].status = 'declined';
res = calculateStandbyCascade(week3Declines);
assert(res.totalOpenSpots === 3, 'Total open spots is 3');
assert(res.springer1.status === 'offered', 'Springer 1 is offered');
assert(res.springer2.status === 'offered', 'Springer 2 is offered');
assert(res.frei.status === 'offered', 'Frei is offered');
assert(res.activeOfferedPrios.length === 3, 'All 3 priority levels offered');
